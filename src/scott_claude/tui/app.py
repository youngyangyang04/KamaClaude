...
class ScottTuiApp(App[None]):
    """ScottClaude TUI：终端滚屏风格，实时展示 agent 执行过程。"""
    ...
    _BANNER = (
        f"[bold yellow]{_BANNER_ART}[/bold yellow]\n"
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]ScottClaude[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    def _update_header(self, state: str) -> None:
        ...
        header.update(
            f"[bold]ScottClaude[/bold]  [dim]{self._host}:{self._port}[/dim]"
    ...
    async def _socket_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                header.update(f"[bold]ScottClaude[/bold]  [red]subscribe error: {e}[/red]")
            finally:
...
# TUI 入口：读取配置并启动 ScottTuiApp
def run(config: ScottConfig, replay_run_id: str | None = None) -> None:
    app = ScottTuiApp(config.host, config.port, replay_run_id=replay_run_id)
    app.run()
